...
> modelo). Numeración de tablas **consecutiva global**, continuando desde el Capítulo 4 (la primera
> tabla de este capítulo es la Tabla 10). Fuente de los datos:
> `outputs/evaluation/final_all_test.csv` y los resúmenes por corrida.
...
La Tabla 10 presenta el Dice por región y el Dice medio de las tres regiones para las siete
configuraciones evaluadas. La Tabla 11 recoge la métrica de frontera HD95 para las mismas.

**Tabla 10.** Coeficiente Dice sobre test (media ± desviación típica de tres semillas; nnU-Net es
...
**Tabla 11.** Distancia HD95 (mm) sobre test por región (media de tres semillas; nnU-Net,
...
Del contraste de arquitecturas (Tablas 10 y 11) se observa una **jerarquía clara**:
...
fusión (§4.3.6). La Tabla 12 desglosa el Dice medio por semilla, lo que resulta esencial para
interpretar la elevada desviación de las variantes adaptativas.

**Tabla 12.** Dice medio sobre test por semilla para las cuatro estrategias de fusión (misma U-Net
...
- **La fusión adaptativa es inestable.** Ambas variantes de compuerta obtienen una media claramente
  ...
  Tabla 12: **cada variante colapsa en una de las tres semillas** —`adaptive_gating` en la semilla
  ...
  **distintas** descarta que sea un problema de una inicialización concreta: la compuerta adaptativa
  ...
  estáticas no presentan. Los HD95 desorbitados de estas dos filas en la Tabla 11 (~23 mm) son
...
La Tabla 13 resume el coste de cada arquitectura. Los modelos residuales (incluidas las variantes
...
**Tabla 13.** Coste computacional por modelo: número de parámetros, entorno de entrenamiento y
